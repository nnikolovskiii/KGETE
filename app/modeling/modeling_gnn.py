...
class GATConvE(MessagePassing):
    ...
    def message(self, edge_index, x_i, x_j, edge_attr):  # i: tgt, j:src
        ...
        src_node_edge_count = torch.zeros(N, dtype=torch.float, device=edge_index.device)
        src_node_edge_count.scatter_add_(0, src_node_index, ones)
        assert len(src_node_edge_count.size()) == 1 and len(src_node_edge_count) == E
        alpha *= src_node_edge_count.unsqueeze(1)  # [E, heads]
